<script src="https://code.jquery.com/jquery-1.11.1.min.js"></script> 
	...
	<%-- div id="suscrip" class="activ">
		<h5><i><span></span></i>Suscríbete al Boletín 
			...
			<div class="wpSus">
				<form name="Principal" id="Principal" method="post" enctype="multipart/form-data" onsubmit="return ValidarHeader();"> 
					...
					<input id="subscribe1" type="submit" value="Suscríbete" name="boton1" />	
					 ...
					 $("#RegistroPrincipal").css("display", "block");
					 	...
					 	if(n==-1){
					 		...
					 		if(y==-1){
					 			...
					 			$("#RegistroPrincipal").html("Su correo ha sido registrado exitosamente");
					 		}
					 		else{
					 			$("#RegistroPrincipal").html("Esta dirección de correo ya se encuentra registrada");
					 		}
					 	}
					 	else if (n>-1){
					 		$("#RegistroPrincipal").html("El correo que ha ingresado es incorrecto");
					 	}
	 ...
	 <%-- div class="bulletin cont ocultarBulletin desktop" id="bulletin">
	 	<div class="titlebulletin borderb">		
	 		...
	 		<div class="titlebulletin txt">Boletines</div>	
	 		<div class="titlebulletin txt">Recibe nuestros boletines de noticias directamente en tu bandeja de entrada</div>
	 		<div class="subsside">
	 			<form name="Bloque" id="Bloque" method="post" enctype="multipart/form-data" onsubmit="return ValidarBloque();" >
	 				<input id="email" class="mail" type="text" placeholder="Correo electrónico" name="email"  maxlength="255" />
	 				<input id="subscribe3" type="submit" value="Suscríbete" name="boton3" />		 
	 			</form>
	 		</div>
				    ...
				    success: function (data){
					 $("#RegistroBloque").css("display", "block");
					 	...
					 	if(n==-1){
					 		var y = data.search("0 relaciones nuevas");
					 		if(y==-1){
					 			$("#RegistroBloque").html("Su correo ha sido registrado exitosamente");
					 		}
					 		else{
					 			$("#RegistroBloque").html("Esta dirección de correo ya se encuentra registrada");
					 		}
					 	}
					 	else if (n>-1){
					 		$("#RegistroBloque").html("El correo que ha ingesado es incorrecto");
					 	}
	 ...
	 <div class="bulletin cont OK desktop" id="bulletin" style="display: none;">
	 	<div class="titlebulletin borderb">
	 		...
	 		<div class="titlebulletin txt">Su suscripción fue realizada satisfactoriamente.</div>
	 	</div>
	 </div>
	 <% } %>	
