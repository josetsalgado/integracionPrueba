...
jQuery.fn.GetVideos = function(){
	...
	.done(function(data){
		...
		alert("adentro de la funcion el parametro es "+parametro+" y la descripcion es "+desc);
...
jQuery.fn.GetCorresponsales = function(){
	...
	.done(function(data){
		$.each(data,function(i){
			...
			titulo		= data[i].titulo;
			imagenOriginal	= data[i].thumbnail_mediano;
			video_slug	= data[i].slug;
...
jQuery.fn.GetVideosDestacados = function(parametro, desc, title){
	
	var path 	= "//multimedia.telesurtv.net/api/clip/?callback=?&"+parametro+"="+desc;
	var categoria	= "";
	var fecha	= "";
	var descripcion = "";
	var $FRAME	= "";
	var $ASIDE	= '<div class="block latest">'+
				'<a style="margin-left: -200px;margin-top:500px" href="http://www.telesurtv.net/seccion/news">'+
	...
	.done(function(data){
		$.each(data,function(i){
			...
			*/
			...
			//$ASIDE += '<div class="wpSmallImg">';
			//$ASIDE += '<a href="javascript:;" onclick="'+watchVideoFN+'"><img src="/arte/ico-play.png" alt="Telesur" class="icoPlay"></a>';
			$ASIDE += '<div style="width:300px;float:left;margin:10px">';
			$ASIDE += '<a href="javascript:;" onclick="'+watchVideoFN+'"><img src="'+imagen+'" height="82" alt="Telesur"></a>';
			//$ASIDE += '</div>';
			$ASIDE += '<div class="overflow">';
			$ASIDE += '<h5>'+categoria+'<span>'+duracion+'</span></h5>';
			$ASIDE += '<h4><a href="javascript:;" onclick="'+watchVideoFN+'">'+titulo.slice(0,50)+'</a></h4>';
			//$ASIDE += '</div>';
			$ASIDE += '</div>';
			$ASIDE += '</div>';
